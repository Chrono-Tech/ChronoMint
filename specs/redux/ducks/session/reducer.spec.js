import reducer from '../../../../src/redux/ducks/session/reducer';
import * as a from '../../../../src/redux/ducks/session/actions';
...
describe('settings cbe reducer', () => {
    ...
    it('should handle SESSION_CREATE_SUCCESS', () => {
        ...
        expect(localStorage.getItem('chronoBankAccount')).toEqual(accounts[0]);
    });
    ...
    it('should handle SESSION_DESTROY', () => {
        /** prepare */
        localStorage.setItem('chronoBankAccount', accounts[0]);
        return UserDAO.watchCBE(() => {}, accounts[0]).then(() => {
            ...
            expect(localStorage.getItem('next')).toEqual('test');
        });
    });
});