...
class LOCDAO extends AbstractContractDAO {
  ...
  getString (setting) {
    return this._call('getString', [Setting.get(setting)]).then(r => this._c.bytesToString(r))
  }

  getValue (setting) {
    return this._call('getString', [Setting.get(setting)]).then(r => this._c.toDecimal(r))
  }
  ...
  loadLOC () {
    ...
    promises.push(
      this._call('getString', [Setting.get('publishedHash')])
        .then(r => callBack('publishedHash', this._c.bytes32ToIPFSHash(r)))
    )
