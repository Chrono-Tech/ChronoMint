export default {
  en: {
    dialogTitle: 'Crowdsale',
    crowdsaleType: 'Crowdsale type',
    timeLimited: 'Time limited',
    blockLimited: 'Block limited',
    startDate: 'Start date',
    fundingTarget: 'Funding target',
    chooseCurrency: 'Choose currency',
    allAvailableCurrencies: 'All Available Currencies',
    minimumValue: 'Minimum value',
    maximumValue: 'Maximum value',
    exchangeRate: 'Exchange rate',
    token: 'Token',
    startPrice: 'Start price',
    currencyAccepted: 'Currency accepted for payment',
    confirmAndStart: 'Confirm and start crowdsale'
  },
  ru: {
    dialogTitle: 'Crowdsale',
    crowdsaleType: 'Тип crowdsale',
    timeLimited: 'Время ограничено',
    blockLimited: 'Блоки ограничены',
    startDate: 'Дата начала',
    fundingTarget: 'Целевой показатель финансирования',
    chooseCurrency: 'Выберите валюту',
    allAvailableCurrencies: 'Все доступные валюты',
    minimumValue: 'Минимальное значание',
    maximumValue: 'Максимальное значение',
    exchangeRate: 'Обменный курс',
    token: 'Токен',
    startPrice: 'Стартовая цена',
    currencyAccepted: 'Валюта, принимаемая к оплате',
    confirmAndStart: 'Подтвердить и запустить crowdsale'
  },
}
