  ...
  renderBlock (block) {
    ...
    return (
      <div styleName='row' key={block.number}>
        <div styleName='column'><Moment date={block.timestamp * 1000} format='Do MMMM YYYY, HH:mm:ss' /></div>
        <div styleName='column'>{block.number}</div>
        <div styleName='column'>LHT <TokenValueSimple value={reward} withFraction /></div>
      </div>
    )
  }

  render () {
    const { wallet, totalRewards, rewardsBlocksList, isListLoading } = this.props
    return (
      <div styleName='root'>
        <div styleName='container'>
          <div styleName='header'>
            <div styleName='title'><Translate value={`${prefix}.title`} /></div>
            <div styleName='subTitle'><Translate value={`${prefix}.subTitle`} />
              {' '}
              <Link
                to='/wallet'
                onClick={() => {
                  this.handleSelectLink(wallet.blockchain, wallet.address)
                }}
              >
          ...
          <div styleName='table'>
            ...
            <div styleName='tableBody'>
              {
                rewardsBlocksList
                  ? Object.values(rewardsBlocksList)
            ...
            <div styleName='tableFooter'>
              <Button
                flat
                label={isListLoading ? <Preloader
                  style={{ verticalAlign: 'middle', marginTop: -2 }}
                  size={24}
                  thickness={1.5}
                /> : <Translate value={`${prefix}.more`} />}
