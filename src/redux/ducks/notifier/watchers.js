...
const watchUpdLOCStatusNotify = () => (dispatch) => LOCsManagerDAO.contract.then(LOCsDeployed => {
    LOCsManagerDAO._watch(LOCsDeployed.updLOCStatus, (r, block, time, isOld) => {
        const status = r.args._status.toNumber();
        dispatch(notifyUpdateLOCValue(r.args._LOC, 'status', status, time, isOld));
    });
});

const watchUpdLOCValueNotify = () => (dispatch) => LOCsManagerDAO.contract.then(LOCsDeployed => {
    LOCsManagerDAO._watch(LOCsDeployed.updLOCValue, (r, block, time, isOld) => {
        ...
        dispatch(notifyUpdateLOCValue(r.args._LOC, settingName, value, time, isOld));
    });
});

const watchUpdLOCStringNotify = () => (dispatch) => LOCsManagerDAO.contract.then(LOCsDeployed => {
...
const notifyUpdateLOCValue = (address, valueName, value, time, isOld) => (dispatch) => {
