...
export const changeNetworkStatus = (status) => (dispatch) => {
  return dispatch({type: CHANGE_NETWORK_STATUS, status})
}

export const changeSyncStatus = (status, progress) => (dispatch) => {
  return dispatch({type: CHANGE_SYNC_STATUS, status, progress})
}
