...
describe('assetsManager reducer', () => {
  ...
  it('should handle GET_ASSETS_MANAGER_COUNTS_START', () => {
    expect(reducer(null, { type: a.GET_ASSETS_MANAGER_COUNTS_START })).toEqual({
  ...
  it('should handle SET_TOKEN', () => {
    const map = new Immutable.Map({ a: 1, b: 2 })
    const map1 = new Immutable.Map({ c: 1, d: 2 })
    const mapRes = new Immutable.Map({ a: 1, b: 2, c: 1, d: 2 })
    expect(reducer({ tokensMap: map }, {
      type: a.SET_TOKEN,
      payload: {
        tokensMap: map1,
        assets: [1, 2, 3, 4],
  ...
  it('should handle SET_WATCHERS', () => {
    expect(reducer({ watchers: { a: 1 } }, {
      type: a.SET_WATCHERS,
      payload: {
        watchers: { b: 1 },
      },
